...
def create_customer_state_df(df):
    customer_state_df = df.groupby("customer_state").customer_id.nunique(
    ).sort_values(ascending=False).reset_index()
...
def create_customer_city_df(df):
    customer_city_df = df.groupby("customer_city").customer_id.nunique(
...
def create_seller_state_df(df):
    seller_state_df = df.groupby("seller_state").seller_id.nunique(
    ).sort_values(ascending=False).reset_index()
...
def create_seller_city_df(df):
    seller_city_df = df.groupby("seller_city").seller_id.nunique(
    ).sort_values(ascending=False).reset_index().head(10)
...
with col1:
    ...
    ax.set_title("Total Order per Bulan (2016-2018)",
...
with col2:
    ...
    ax.set_title("Total Revenue per Bulan (2016-2018)",
                 loc="center", fontsize=20)
    ax.tick_params(axis='y', labelsize=20)
    ax.tick_params(axis='x', labelsize=15, rotation=45)
    st.pyplot(fig)
...
sns.barplot(x="order_item_id", y="product_category_name", data=sum_order_items_df.sort_values(
    by="order_item_id", ascending=False).head(5), palette=colors, ax=ax[0])
ax[0].set_ylabel(None)
ax[0].set_xlabel(None)
ax[0].set_title("Produk yang Paling Banyak Diminati",
                loc="center", fontsize=15)
ax[0].tick_params(axis='y', labelsize=12)

sns.barplot(x="order_item_id", y="product_category_name", data=sum_order_items_df.sort_values(
    by="order_item_id", ascending=True).head(5), palette=colors, ax=ax[1])
ax[1].set_ylabel(None)
ax[1].set_xlabel(None)
ax[1].invert_xaxis()
ax[1].yaxis.set_label_position("right")
ax[1].yaxis.tick_right()
ax[1].set_title("Produk yang Paling Sedikit Diminati",
                loc="center", fontsize=15)
...
ax.set_title("Demografi Pelanggan Berdasarkan Negara",
             loc="center", fontsize=15)
ax.set_ylabel(None)
ax.set_xlabel(None)
...
with col1:
    ...
    sns.barplot(
        x="customer_count",
        y="customer_city",
        data=customer_city_df.sort_values(
            by="customer_count", ascending=False),
        palette=colors_
    )
    ax.set_title("10 Kota dengan Pelanggan Paling Banyak",
                 loc="center", fontsize=15)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
    ax.tick_params(axis='y', labelsize=12)
...
with col2:
    ...
    ax.set_title("10 Kota dengan Pelanggan Paling Sedikit",
                 loc="center", fontsize=15)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
    ax.tick_params(axis='y', labelsize=12)
...
with col1:
    ...
    ax.set_title("10 Kota dengan Penjual Paling Banyak",
                 loc="center", fontsize=15)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
    ax.tick_params(axis='y', labelsize=12)
...
with col2:
    ...
    ax.set_title("10 Kota dengan Penjual Paling Sedikit",
                 loc="center", fontsize=15)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
    ax.tick_params(axis='y', labelsize=12)
